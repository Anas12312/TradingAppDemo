...
export default function InTradeTableBody({ searchedRecords, records, header, setSelectedTicker, setRecords, setSearchedRecords }) {
        ...
        <div className='w-full h-[70%] relative'>
            ...
            <div className=' h-full [&>*:nth-child(even)]:bg-white [&>*:nth-child(odd)]:bg-blue-100
                ...
                {searchedRecords?.map((record, i) => {
                    if(i % 2) return
                    return (
                        <InTradeTableRow setContextRow={setContextRow} setClicked={setClicked} setPoints={setPoints} key={i} record={record} i={i} header={header} setSelectedTicker={setSelectedTicker} setRecords={setRecords} setSearchedRecords={setSearchedRecords} records={records} searchedRecords={searchedRecords} />
                    ) 
                })}
            </div>
            {clicked && (
