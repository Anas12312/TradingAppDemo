...
export default function TableRow({ setContextRow, setClicked, setPoints, record, i, selectedTicker, header, setSelectedTicker, setRecords, setSearchedRecords, records, searchedRecords, setIdle, timer }) {
    ...
    return (
        <tr onContextMenu={(e) => {
            ...
            <td className={'w-[5%] h-full flex justify-center items-center text-center truncate border-r border-blue-700 ' + (!record.s3 ? '' : (record.s2 === 1 ? ' bg-red-500' : '') + (record.s2 === -1 ? ' bg-green-400' : ''))}>
                {record.s3}
            </td>
